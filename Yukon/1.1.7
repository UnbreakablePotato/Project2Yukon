...
char createDeckFile(char filename[]) {
    ...
    //Check if file exist
    if (file == NULL) {
        print(head, "LD", "ERROR: no existing file.");
        return 0;
    }
    ...
    for (int i = 0; i < 52; i++) {
        if (strcmp(Deckarray[i], "") == 0) {
            print(head, "LD", "ERROR deck is too small");
            return 0;
        }
    }
    ...
    if (strcmp(Extra, "") != 0) {
        print(head, "LD", "ERROR deck is too big");
        return 0;
    }
