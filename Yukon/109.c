...
void P() {
    ...
    //TODO ændre print funktionen, så vi får 7 lister printet korrekt.
    Node* temp = head;
    int line = 0;
    printf("c1\tc2\tc3\tc4\tc5\tc6\tc7\n\n");
    for (int row = 0; row < 8; row++) {
...
void GameMoves(char* Operation) {
    ...
    char* card;
    char* load = Operation;
    char* from = strtok(load, "->");
    char* to = strtok(NULL, "->");

    if (strlen(from) > 2) {
        char* from1 = from;
        char* column = strtok(from1, ":");
        card = strtok(NULL, ":");
        from = column;
    }

    if (strcmp(from,"C1") == 0) {
        Node* currentFrom = head;
    } else if (strcmp(from, "C2") == 0) {
        Node* currentFrom = SecondHead;
    } else if (strcmp(from, "C3") == 0) {
        Node* currentFrom = ThirdHead;
    } else if (strcmp(from, "C4") == 0) {
        Node* currentFrom = FourthHead;
    } else if (strcmp(from, "C5") == 0) {
        Node* currentFrom = FifthHead;
    } else if (strcmp(from, "C6") == 0) {
        Node* currentFrom = SixthHead;
    }else if (strcmp(from, "C7") == 0) {
        Node* currentFrom = SeventhHead;
    }else if (strcmp(from, "F1") == 0) {
        Node* currentFrom = Fone;
    }else if (strcmp(from, "F2") == 0) {
        Node* currentFrom = Ftwo;
    }else if (strcmp(from, "F3") == 0) {
        Node* currentFrom = Fthree;
    }else if (strcmp(from, "F4") == 0) {
        Node* currentFrom = Ffour;
    }

    if (strcmp(to, "C1") == 0) {
        Node* currentTo = head;
    } else if (strcmp(to, "C2") == 0) {
        Node* currentTo = SecondHead;
    }else if (strcmp(to, "C3") == 0) {
        Node* currentTo = ThirdHead;
    }else if (strcmp(to, "C4") == 0) {
        Node* currentTo = FourthHead;
    }else if (strcmp(to, "C5") == 0) {
        Node* currentTo = FifthHead;
    }else if (strcmp(to, "C6") == 0) {
        Node* currentTo = SixthHead;
    }else if (strcmp(to, "C7") == 0) {
        Node* currentTo = SeventhHead;
    }else if (strcmp(to, "F1") == 0) {
        Node* currentTo = Fone;
    }else if (strcmp(to, "F2") == 0) {
        Node* currentTo = Ftwo;
    }else if (strcmp(to, "F3") == 0) {
        Node* currentTo = Fthree;
    }else if (strcmp(to, "F4") == 0) {
        Node* currentTo = Ffour;
    }
    ...
    if (card != NULL) {
        while (currentFrom != NULL && strcmp(currentFrom->value, card) == 0) {
        ...
        //assigning the rank and suit from the "from" and "to" list to variables for easier validation checks
        currentTo->value = toCheck;
        if (strcmp(toCheck[1][1], "A")==0 || strcmp(toCheck[1][1], "J")==0 || strcmp(toCheck[1][1], "Q")==0
            || strcmp(toCheck[1][1], "K")==0 || strcmp(toCheck[1][1], "T")==0) {
            rankTo = toCheck[1][1];
            }else {
                rankTo = atoi(toCheck[1][1]);
            }
        char suitTo = toCheck[1][2];

        currentFrom->value = fromCheck;
        if (strcmp(fromCheck[1][1], "A")==0 || strcmp(fromCheck[1][1], "J")==0 || strcmp(fromCheck[1][1], "Q")==0
            || strcmp(fromCheck[1][1], "K")==0 || strcmp(fromCheck[1][1], "T")==0) {
            rankFrom = fromCheck[1][1];
        }else {
            rankFrom = atoi(fromCheck[1][1]);
        }

        char suitFrom = fromCheck[1][2];



        if (strcmp(to, "F1") == 0 || strcmp(to, "F2") == 0 || strcmp(to, "F3") == 0 ||strcmp(to, "F4") == 0) {
            //Checking if card moving to foundation is the card bottom of column (foundation validation)
            if (currentFrom->next != NULL) {
                //print error card isn't the most bottom card
            }
            //Checking if card moving to foundation is one rank higher and same suit (foundation validation)
            if (strcmp(rankFrom, "A")==0 && currentTo != NULL) {
                //print error rank isn't one higher
            }else if (rankFrom == 2 && strcmp(rankTo, "A")!=0) {
                //print error rank isn't one higher
            }else if (strcmp(rankTo, "Q") == 0 && strcmp(rankFrom, "K") != 0) {
                //print error rank isn't one higher
            }else if (strcmp(rankTo, "K") == 0) {
                //print error foundation full
            }else if (strcmp(rankFrom, "Q") == 0 && strcmp(rankTo, "J") != 0) {
                //print error rank isn't one higher
            }else if (strcmp(rankFrom, "J") == 0 && strcmp(rankTo, "T") != 0) {
                //print error rank isn't one higher
            }else if (strcmp(rankFrom, "T") == 0 && rankTo != 9) {
                //print error rank isn't lower
            }else {
            ...
            if (strcmp(suitFrom, suitTo)!=0) {
                //print error the suit is not the same
            }
        }else {
            //Checking if the "from" card is one rank lower and a different suit (column validation)
            if (strcmp(suitTo, suitFrom)==0) {
                //print error the suit is the same
            }

            if (strcmp(rankTo, "A")==0) {
                //print error rank isn't lower
            }else if (strcmp(rankFrom, "A")==0 && rankTo != 2) {
                //print error rank isn't lower
            }else if (strcmp(rankTo, "K") == 0 && strcmp(rankFrom, "Q") != 0) {
                //print error rank isn't lower
            }else if (strcmp(rankFrom, "K") == 0 && currentTo != NULL) {
                //print error rank isn't lower
            }else if (strcmp(rankTo, "Q") == 0 && strcmp(rankFrom, "J") != 0) {
                //print error rank isn't lower
            }else if (strcmp(rankTo, "J") == 0 && strcmp(rankFrom, "T") != 0) {
                //print error rank isn't lower
            }else if (strcmp(rankTo, "T") == 0 && rankFrom != 9) {
                //print error rank isn't lower
            }else {
