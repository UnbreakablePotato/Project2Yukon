...
void P() {
    ...
    //we only do this once for the whole program, so when a card is revealed, it stays revealed.
    ...
    if (card != NULL) {
        ...
        if (GameMovesCheck(currentFrom, currentTo, to)) {
            ...
            if (currentFrom->prev != NULL) {
                currentFrom->prev->next = NULL; //Set currentFrom prev's next pointer to NULL
