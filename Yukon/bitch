            ...
            if ((Operations == 1) && (strcmp(Operation, "LD") == 0)) {
                ...
                while (strcmp(playOperation, "Q") != 0) {
                    ...
                    if (fgets(playInput, sizeof(playInput), stdin) != NULL) {
                        ...
                        if ((playOperation[0] == 'C' || playOperation[0] == 'F') && strlen(playOperation)>4){
                            backupGameState();
                            GameMoves(playOperation);
                            undoFlag=1;
                        }else if ((strcmp(playOperation, "U") == 0) && undoFlag == 1) {
                            undoGameState();
                            undoFlag=0;
                            ...
                            "Message: Unknown Command\n"
                            "INPUT >", playOperation);
                        } else if (strlen(playOperation) > 1) {
                            printf("LAST Command: %s\n"
                            "Message: Unknown Command\n"
                            "INPUT >", playOperation);
                        }
            ...
            for (int j = 0; j < 52; j++) {
                if (i != j) {
                    if (strcmp(Deckarray[i], Deckarray[j])==0) {
                        ...
                        "\t\t\t\t\t\t\t[]\tF4\n\n"
                        "LAST Command: LD \n"
                        "Message: ERROR duplicate detected: %s at line %d\n"
                        "INPUT >", Deckarray[j], j+1);
                        return 0;
                    }
                }
            }
...
Node* mergeAlt(Node* head, Node* SecondHead) {
    if (!head) {
        return SecondHead; //tjek til at se om head er tom. Hvis ja så return den anden liste, da der ikke er grund til at interweave
    }
    ...
    Node* mergedHead = pointer;

    while (pointer && pointer2) { //loop så længe ingen af listerne er tomme
        Node* next1 = pointer->next; // gemmer next noder i en next1 og next2
        Node* next2 = pointer2->next;

        pointer->next = pointer2;// laver et liink for næste node
        pointer2->prev = pointer;

        if (next1) { //hvis der er next1 så, er pointer2's next lig next1 og vi linker den tilbage til pointer2 med prev
            pointer2->next = next1;
            next1->prev = pointer2;
        }
...
void GameMoves(const char* Operation) {
    ...
    if (strcmp(from,"C1") == 0) {
        ...
        GameMovesPrint();
        printf("LAST Command: %s\nMessage: Error - source list not found\nINPUT >", Operation);
        return;
    }
    ...
    if (strcmp(to, "C1") == 0) {
        ...
        GameMovesPrint();
        printf("LAST Command: %s\nMessage: Error - destination list not found\nINPUT >", Operation);
        return;
    }
