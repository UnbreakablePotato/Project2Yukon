...
void randomNumber();
void shuffleRandom(Node* head, Node* SRHead);
...
void shuffleRandom(Node* head, Node* SRHead) { // skal vælge et random kort fra decket og sætte på position x i et nyt deck
    if (head==NULL) {
        printf("c1\tc2\tc3\tc4\tc5\tc6\tc7\n"
           "\t\t\t\t\t\t\t[]\tF1\n\n"
           "\t\t\t\t\t\t\t[]\tF2\n\n"
    ...
    for (int j=0;j<52;j++) {
        ...
        for (int i=0;i<=rNumArray[j];i++) {
            ...
            //logikken kører slet ikke endnu

            if (i<rNumArray[j]) { //Vi trækker os gennem listen og gemmer den i next1
                pointer = pointer->next;
            }

            else if (i==rNumArray[j]) {
                    if (SRHead == NULL) {
                        // TODO Create new node or reuse node
                       // Node* newCard = (Node*)malloc(sizeof(Node));
                        //newCard = pointer->value;
                        Node* newCard = createCard(pointer->value);
                        SRHead=newCard->value;
                    }

                    else if (pointer) { //logik til at gøre det modsatte, så link2->link1->link2 men kun hvis der er et link1
                        // TODO Create new node or reuse node
                        //Node* newCard = (Node*)malloc(sizeof(Node));
                        //newCard = pointer->value;
                        Node* newCard = createCard(pointer->value);
                        SRHead->next = newCard->value;
                    } else {
                        // TODO Create new node or reuse node
                        //Node* newCard = (Node*)malloc(sizeof(Node));
                        //newCard = pointer->value;
